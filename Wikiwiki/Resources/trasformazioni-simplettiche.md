...
* **D** : Funzione dif $g:U->R^{2d}$ è *simplettica* se per ogni punto di $U$, $w(J(g)u,J(g)v)=w(u,v)$ per ogni $u,v$
    ...
    - interpretazione geometrica: M sottovarietà 2d immagine di K (in $R^2$) tramite f(s,t). M è unione di parallelogrammi generati da
...
* **T** (Lemma di integrabilità) : Sia $g:U->R^n$ dif con continuità e sia Jg(y) simmetrica per tutti gli y in U, allora
...
*  **T** : $g:U->R^{2d}$ dif con continuità, allora $\dot{y}=f(y)$ è localmente hamiltoniano se e solo se il suo flusso $f_t(y)$ è simplettico
